...
class Supervisor extends Component{
    ...
    componentDidMount () {
        this.setState({
            groups: isAuthenticated().user.supervisorgroup
        })
    }
            ...
            { groups.map((group,i) => (
                (group.id && <div className="card" style={{height: "100%", width: "35%", marginLeft: "10%", marginTop: "5%"}} key={i}>
                    <div className="card-body" >
                        ...
                        <Link to={`/supervisor/${group._id}`} className="btn btn-raised btn-primary btn-sm" style = {{marginLeft: "32%", marginTop: "5%", fontWeight: 400, fontSize: 16}}>View Group</Link>
                    </div>
                </div>
                )
            ))}
