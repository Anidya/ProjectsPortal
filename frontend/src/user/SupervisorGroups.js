...
class SupervisorGroups extends Component{
    ...
    renderUsers = (groups) => (
        <div className="row">
            { groups.map((group,i) => (
                ( (group.supervisors[0].email === isAuthenticated().user.email  || group.supervisors[1].email === isAuthenticated().user.email) && (
                    <div className="card" style={{height: "400px", width: "350px", marginLeft: 250, marginTop: 50}} key={i}>
                        <div className="card-body" >
                            ...
                            <Link to={`/supervisor/${group._id}`} className="btn btn-raised btn-primary btn-sm" style = {{marginLeft: 90, marginTop: 25, fontWeight: 400, fontSize: 16}}>View Group</Link>
                        </div>
                    </div>
                ))
            ))}
